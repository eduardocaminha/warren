...
3. **Seed the burrow** — `buildSeedFiles(agent)` returns an `HttpWorkspaceFile[]` covering the seven workspace drops (`.canopy/agent.json`, `.mulch/expertise/<domain>.jsonl` from `expertise_seed`, `.seeds/workflow.txt`, `.pi/skills/<name>/SKILL.md` from `pi_skills`, `.pi/prompts/<name>.md` from `pi_prompts`, `.pi/extensions/<name>.ts` from `pi_extensions`, `.mcp.json` from `mcp_servers`). The list rides along on the step-2 `POST /burrows` as `seed.files`, so provision-and-seed is a single atomic round-trip — burrow rolls the burrow back on its side if any file fails validation, and warren never observes a half-seeded workspace (R-07; see §11.A).
