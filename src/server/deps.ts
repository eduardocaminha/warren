/**
 * `ServerDeps` injection bag plus the `Logger` and `BridgeRegistry` types it
 * depends on (warren-e0da: extracted from `types.ts` to keep both files under
 * the 500-line threshold). Re-exported from `types.ts` for backwards
 * compatibility — callers that import from `types.ts` are unaffected.
 */
